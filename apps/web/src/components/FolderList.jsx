  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 w-10 h-10 flex flex-col items-center justify-center gap-1.5 bg-sidebar border border-border rounded-md shadow-lg"
        aria-label="Toggle menu"
      >
        <span className={`w-5 h-0.5 bg-foreground transition-all ${isOpen ? 'rotate-45 translate-y-2' : ''}`} />
      ...
      {/* Sidebar */}
      <aside className={`w-64 h-full bg-sidebar flex flex-col lg:relative fixed top-0 left-0 z-40 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        {/* Compose Button - Hidden on mobile (using floating button instead) */}
        ...
        {/* Folders */}
        <nav className="flex-1 px-2" onClick={() => setIsOpen(false)}>
          {folders.length === 0 ? (
            ...
            <div className="space-y-0.5">
              ...
              {sortedFolders.map((folder) => {
                ...
                return (
                  <button
                    key={folder.path}
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-base transition-colors cursor-pointer ${isSelected
        ...
        {/* Links + Settings - Hidden on mobile */}
        <div className="p-2 border-t border-sidebar-border space-y-2 hidden lg:block">
          <a
